...
// 2018-09-18 09:48:18.173408 -0700 PDT m=+0.004151706
package writers
...
var (
	contextTempl *text.Template
	handlerTempl *text.Template
	pageGoTempl *text.Template
	pageTempl *html.Template
	partialTempl *html.Template
	startTempl *text.Template
	
)

func init() {
	var err error
	contextTempl, err = text.New("templates/context.go.templ").Parse($template.Ident)
	if err != nil {
		log.Fatal(err)
	}
	handlerTempl, err = text.New("templates/handler.go.templ").Parse($template.Ident)
	if err != nil {
		log.Fatal(err)
	}
	pageGoTempl, err = text.New("templates/page.go.templ").Parse($template.Ident)
	if err != nil {
		log.Fatal(err)
	}
	pageTempl, err = html.New("templates/page.templ.html").Delims("[[", "]]").Parse($template.Ident)
	if err != nil {
		log.Fatal(err)
	}
	partialTempl, err = html.New("templates/partial.templ.html").Delims("[[", "]]").Parse($template.Ident)
	if err != nil {
		log.Fatal(err)
	}
	startTempl, err = text.New("templates/start.go.templ").Parse($template.Ident)
	if err != nil {
		log.Fatal(err)
	}
